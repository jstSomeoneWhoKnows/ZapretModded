            ...
            if not "!arg!"=="^" (
                ...
                if "!arg:~0,1!" EQU "!QUOTE!" (
                    ...
                    if !errorlevel!==0 (
                        ...
                        rem Заглушка, чтобы не ломать логику раскрытия %%
...
if !winws_running! neq 0 if !windivert_running!==0 (
    call :PrintYellow "[?] winws.exe не запущен, но служба WinDivert active. Попытка удаления WinDivert..."
...
if "%IPsetStatus%"=="ТОЛЬКО ЗАГРУЖЕННЫЕ" (
    ...
    if exist "%backupFile%" (
        ...
        echo Ошибка: файл ipset-all.txt.backup не найден в папке lists.
        echo Пожалуйста, обновите списки через пункт 7 в меню.
        echo:
    )
)
...
set "listFile=%~dp0lists\ipset-all.txt"
set "url=https://raw.githubusercontent.com/jstSomeoneWhoKnows/ZapretModded/refs/heads/main/lists/ipset-all.txt"
...
:: Вспомогательные функции (Ускоренные через ANSI ESC-последовательности)
:PrintGreen
echo [32m%~1[0m
exit /b
