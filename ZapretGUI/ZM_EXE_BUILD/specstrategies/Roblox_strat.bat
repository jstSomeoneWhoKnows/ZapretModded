...
:: Roblox-специфичные настройки TCP

--filter-tcp=80,443 --hostlist="%LISTS%roblox\list.txt" --dpi-desync=fake,fakedsplit --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fakedsplit-pattern=0x00 --dpi-desync-fake-tls="%BIN%tls_clienthello_www_google_com.bin" --dpi-desync-fake-http="%BIN%tls_clienthello_max_ru.bin" --new ^


:: Roblox-специфичные настройки UDP

--filter-udp=443,49152-65535 --ipset="%LISTS%roblox\ipset.txt" --dpi-desync=fake --dpi-desync-repeats=10 --dpi-desync-any-protocol=1 --dpi-desync-fake-quic="%BIN%quic_initial_www_google_com.bin" --dpi-desync-cutoff=n2 --new ^


:: Настройки для общего трафика
